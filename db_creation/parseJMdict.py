...
tree = eTree.parse('database/JMdict_e.xml')
root = tree.getroot()

conn = sqlite3.connect('database/entireDict.db')
cursor = conn.cursor()
...
for ent in root.findall('entry'):
    ...
    if reading_elements:
        for r in range(len(reading_elements)):
            lastRRow = ins.insertReadingElement(reading_elements[r],entLastRow,cursor)
            ins.insertAllReadingEle(reading_elements[r],lastRRow,cursor)
